...
$delay_ns = isset($_GET['ns']) ? 0+$_GET['ns'] : 1000;
$time_sec = isset($_GET['sec']) ? 0+$_GET['sec'] : 30;
...
$objects = select("select id, device_id, Object_Identifier, Object_Type, -1+Object_Type as typeId, `value`, status, Object_Name from objects where Object_Name LIKE 'Site:Engineering/Electricity.%' AND Object_Type in ('".join("','",$data_types)."') limit 0,5000");
...
while (time()-$t1<$time_sec) {
    ...
    $value0 = 0+$VAL0[$id];
    $newValue =  rand($value0*85,$value0*115)/10 + rand(0,$value0/10);
