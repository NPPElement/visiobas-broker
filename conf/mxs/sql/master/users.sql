...
BEGIN
	...
	DECLARE currVal 	DOUBLE;
	DECLARE newVal 	DOUBLE;
	
	DECLARE flag INT DEFAULT 0;
	DECLARE cur CURSOR FOR SELECT id, Low_Limit, High_Limit FROM `objects.analog-input` WHERE Low_Limit IS NOT NULL AND High_Limit IS NOT NULL;
	DECLARE CONTINUE HANDLER FOR NOT FOUND SET flag = 1;

	OPEN cur;
	
 	REPEAT
 		FETCH cur INTO objId, lowLimit, highLimit;
 		
 		call func_debug( CONCAT("fetch: ", objId));
 		
 		SET randVal = RAND();
 		SELECT value FROM objects WHERE id=objId INTO currVal;
 		IF currVal > highLimit THEN
 			SET newVal = currVal-randVal;
 		ELSEIF currVal < lowLimit THEN
 			SET newVal = currVal+randVal;
		ELSE 
			SET newVal = currVal+2*randVal-1;
 		END IF;
 		
	UPDATE objects SET value=newVal WHERE id=objId;
...
CREATE DEFINER=`root`@`localhost` EVENT `timer_random_update_objects` ON SCHEDULE EVERY 60 SECOND STARTS '2020-12-15 21:34:55' ON COMPLETION NOT PRESERVE ENABLE DO BEGIN
	...
	DECLARE flag INT DEFAULT 0;
-- 	DECLARE cur CURSOR FOR SELECT id, Low_Limit, High_Limit FROM `objects.analog-input` WHERE Low_Limit IS NOT NULL AND High_Limit IS NOT NULL;
	DECLARE cur CURSOR FOR SELECT id FROM `objects` WHERE Object_Type='analog-input' OR Object_Type='analog-value';
	DECLARE CONTINUE HANDLER FOR NOT FOUND SET flag = 1;
